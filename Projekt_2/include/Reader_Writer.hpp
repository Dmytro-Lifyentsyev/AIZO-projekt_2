...
public:
    Writer(const std::string& filename) {
        file.open(filename); // Otwiera plik (nadpisuje poprzednie wyniki)
        if (!file.is_open()) {
            std::cerr << "Blad: Nie udalo sie otworzyc pliku do zapisu: " << filename << "\n";
        }
    }
    ...
    // Zwraca referencje do pliku. Jeśli plik się nie otworzył, zwraca konsole.
